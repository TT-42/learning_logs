...
- BGPの優先順位の設定
    - ローカルプリファレンスのBGPコミュニティタグを使用する
        - 最も高い値が優先される
    - AS_PATHは経路選択に使用する値
        - AS_PATH prepend属性を設定することによってプライマリよりも経路の優先度を下げることができる
...
- BGPパス選択の優先度
    1. 最長プレフィックス一致
        - ルーティングテーブル内のプレフィックス長が最も長いルート
    1. DirectConnect BGPルート
        - AWS DirectConncect接続からBGPで伝搬されたルート
    1. スタティックルート
    1. VPN BGPルート
## NLB
- NLBのクロスゾーン負荷分散
...
- AWS Global AcceleratorとNLBを使用すると、アプリケーションに静的IPアドレスを提供できるが、クライアントの送信元IPアドレスを保持する機能が失われる
    - NLBがクライアントの送信元IPアドレスを転送せず、代わりにロードバランサーのIPアドレスを使用するため
## AWS Direct Connect
- 基礎知識
...
- AWS Direct Connect SiteLink
    ...
    - **既存のトランジットVIFで有効にする**ことにより、新たな設備投資や追加のゲートウェイ構築を行うことなく、即時に高い接続信頼性を実現できる
## Transit Gateway
- Transit Gateway（以下TGW）の基礎知識
    - 用語
        - アタッチメント
            - TGWにVPC・VPN・Direct Connectを関連付けるアクション
                - 対象をTGWで管理するための最初のアクション
        - ルートテーブル
            - 1つのTGWあたりに作成可能なルートテーブルは20まで
        - アソシエーション
            - アタッチメントとルートテーブルを関連付ける作業
                - アタッチメント間での通信が可能になる
            - 1つのアタッチメント（1つのVPC）が複数のルートテーブルにアソシエーションすることはできない
        - プロパゲーション
            - アタッチメントしたVPCをルートテーブルに伝搬する機能
                - これによりスタティックルートを定義せず自動でルートテーブルを更新できる
            - TGW Peeringはプロパゲーションできない
        - デフォルト動作
            - デフォルトの設定では以下の設定が有効になっている
                - デフォルトルートテーブルの関連付け
                - デフォルトルートテーブル伝搬
            - この設定が有効になっていると、アタッチメントしたVPCがデフォルトで作成されるルートテーブルに、自動でアソシエーションとプロパゲーションされる
    - 気を付けるポイント
        - VPC側のルートテーブル編集
            - 通信させたいサブネットのルートテーブルで、アタッチメントをターゲットとして設定する必要がある
        - アソシエーションの付け替え
            - アソシエーションを異なるルートテーブルに付け替える場合、ダウンタイムが発生する
        - TGW越しのセキュリティグループ参照
            - TGW越しのVPC間では、セキュリティグループの参照はサポートされない
            - そのため、通信させたい宛先のCIDRを指定する必要がある
        - TGW専用サブネットの作成
            - AWSベストプラクティス
            - アタッチメント作成時にサブネットの指定が必要になり、指定したサブネットにENIが作成されるため
...
- AWS Transit Gateway Network Manager
    - Transit Gatewayに接続された複数のVPCの可視性を向上させるためのツール
        - CloudWatch Logs InsightsとEventBridgeと組み合わせることでルート変更の通知を提供できる
...
- Direct ConnectとSite-to-Site VPNを組み合わせるには、**パブリック仮想インターフェイス**が必要
    - Site-to-Site VPNのエンドポイントにはAWSのパブリックIPアドレスを介さなければならないため
    - プライベート仮想インターフェイスではない
- トンネルAをトンネルBより優先する設定
    - VPN接続をアクティブ/パッシブ構成で設定し、トンネルAにより具体的なプレフィックスをアドバタイズする
- TransitGatewayと接続することでアクセラレーションを有効化することができる
## Route 53
- Route53でのDNSSEC署名の設定
    1. Route53においてDNSSECを有効にする
    1. AWS KMSを用いてキー署名キー（KSK）を作成する
        - us-east-1リージョンに配置されたECC_NIST_P256キー仕様の非対称カスタマー管理キーが必要
...
- AWS Cloud WAN
    ...
    - コアネットワークポリシーを更新し、**セグメント共有を設定する**ことにより、セグメント間のトラフィックフローが可能になる
- MACセキュリティ
    - DirectConnect接続にMACsecを構成することで、高速な暗号化が提供される
    - MACsecキー
...
- 仮想ルータ
    - 高いルーティング能力と大規模なルートセットへの対応力を持つ
    - 例
        - Cisco Cloud Services Router 1000V Series
- S3ゲートウェイエンドポイントはオンプレミスからの接続をサポートしていない
- CloudFrontでHTTPSを使用する方法
    - AWS Certificate Manager（ACM）を使用して、CloudFrontとALBの証明書を作成し、これらのサービスに適用する
    - EC2インスタンス用にサードパーティの証明書プロバイダーからパブリック証明書を取得し、バックエンドでHTTPSリスナーを使用する