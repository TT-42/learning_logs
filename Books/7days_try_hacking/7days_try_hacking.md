...
- AS-REP Roasting攻撃
    ...
    - 今回はImpacketというツールを使用し、TGTを入手した
        - Impacket
            - SecureAuthが開発し、現在はFortra’s Core SecurityによってメンテナンスされているオープンソースのPythonライブラリおよびツールセット
- `Kerbrute`
...
- Pass the Hash攻撃
    ...
    - 今回はEvil-WinRMを利用した
        - Windows Remote Management（WinRM）サービスを利用してリモートからWindowsサーバーにアクセスし、PowerShellインタラクティブシェルを提供するツール

## Blog

- ステガノグラフィ
    - あるデータを他のデータの中に隠す技術
        - 画像、音声、動画などのメディアファイルに情報を埋め込むなど
    - `steghide`
        - 画像や音声ファイルに対して情報を埋め込んだり、取り出したりすることができるツール
- WPScan
    - WordPressサイトのセキュリティを外部から自動で診断するためのオープンソースのブラックボックス型スキャナツール
    - 主な機能
        - WordPress本体・プラグイン・テーマのバージョンと脆弱性の検出
        - ユーザー名の列挙
        - パスワードのブルートフォース攻撃
        - バックアップファイルやエラーログの検出
- Exploit Database
    - 公知の脆弱性を悪用するためのエクスプロイト（攻撃コード）が集約されているサイト
- LinPEAS
    - ターゲットの初期侵入が終わった後に実行することで権限昇格できそうな設定ミスを教えてくれるシェルスクリプト
- ローカルマシンのファイルをターゲットマシンに送る方法
    - 対象ファイルを置いたディレクトリを簡易Webサーバとして公開する
        - `python3 -m http.server 12345`
            - `python3`：Python3のインタプリタを起動する。
            - `-m http.server`：Pythonの標準ライブラリにある `http.server` モジュールを「モジュールとして実行」する。このモジュールは、ローカルディレクトリをWebサーバーとして公開するための機能。
            - 12345：サーバーが待ち受けるポート番号を指定します。
